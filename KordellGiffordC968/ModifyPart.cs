...
namespace KordellGiffordC968
{
    public partial class ModifyPart : Form
    {
        ...
        {
            var part = Inventory.lookupPart(Inventory.AllParts[Inventory.IndexParts].PartID);
            if (part != null)
            {
                idText.Text = part.PartID.ToString();
                ...
                try
                {
                    machineTxt.Text = ((Inhouse)part).MachineID.ToString();
                }
                catch (InvalidCastException)
                {
                    companyNameTxt.Text = ((Outsourced)part).CompanyName;
                }
            }
            else
            {
                MessageBox.Show("Unable to find selected part.");
            }
        }

        #region Save Cancel Controls
